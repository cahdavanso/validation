...
class CONSIGI_KONEXIA:
    ...
    def trata_averbacao(self):
        ...
        try:
            ...
            oper_liq['Nº OPERAÇÃO EDITADO'] = oper_liq['Nº OPERAÇÃO EDITADO'].astype(str)
            oper_liq.to_excel(os.path.join(self.caminho, F"CASOS LIQUIDADOS DE {self.convenio} {self.consignataria}.xlsx"), index=False)
        ...
        prepara_data_averbados = TRATA_CONTRATOS(front_semi_trabalhado=self.front_semi_trabalhado, averbados=data_averbados_bruto, convenio=self.convenio,
                                                 conciliacao_tratada=self.conciliacao, nome_coluna_cpf='CPF', nome_coluna_contrato='NRO CONTRATO',
                                                 nome_coluna_parcela='VALOR DA PARCELA')
